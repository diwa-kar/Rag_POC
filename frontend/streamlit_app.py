...
# st.set_page_config(page_title="Manpower PDF Chatbot", layout="wide")
...
#                     )
#                     st.text(src.get("content", src.get("snippet", "")))
...
for chat in st.session_state.chat_history:
    with st.chat_message(chat["role"]):
        ...
        # Show full sources for assistant
        if chat["role"] == "assistant" and "sources" in chat:
            with st.expander("📚 Sources"):
                for src in chat["sources"]:
                    st.markdown(
                        f"### 📄 Page {src['page']} | Chunk {src['chunk_id']}"
                    )
